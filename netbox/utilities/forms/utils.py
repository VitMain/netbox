...
def restrict_form_fields(form, user, action='view'):
    ...
    # Declared selectors (e.g. a GenericObjectChoiceField scope) resolve their current value from the instance, not
    # the bound form field. On submit a GFK field's object queryset collapses to an empty placeholder that is not a
    # narrowed RestrictedQuerySet, so the loop above skips it; record declared selectors here as well so a hidden
    # current value is still evaluated for preservation. prepare_restricted_queryset_fields() locks only when hidden.
    for name in getattr(form, 'restricted_related_selectors', {}):
        field = form.fields.get(name)
        if field is not None and name not in restricted_fields:
            restricted_fields[name] = getattr(field, 'queryset', None)
