...
class RestrictedTypeDrivenSelectorTest(TestCase):
    ...
    def test_service_parent_hidden_is_preserved(self):
        ...
        form = ServiceForm(
            data={'name': 'svc', 'protocol': 'tcp', 'ports': '80'},
            instance=service,
        )
    ...
    def test_service_parent_type_tamper_is_blocked(self):
        """Submitting a different parent type/object while the current parent is hidden cannot drop or remap it."""
        service = Service.objects.create(name='svc', protocol='tcp', ports=[80], parent=self.device)
        vm_ct = ObjectType.objects.get_for_model(VirtualMachine)

        form = ServiceForm(
            # Tampered: claim the parent is now a VirtualMachine, with no object selected.
            data={
                'name': 'svc', 'protocol': 'tcp', 'ports': '80',
                'parent_content_type': vm_ct.pk, 'parent_object_id': '',
            },
            instance=service,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_vlangroup_scope_hidden_is_preserved(self):
        ...
        form = VLANGroupForm(
            data={'name': 'Group 1', 'slug': 'group-1', 'vid_ranges': '1-100'},
            instance=group,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_circuittermination_hidden_is_preserved(self):
        ...
        form = CircuitTerminationForm(
            data={'circuit': circuit.pk, 'term_side': 'A'},
            instance=termination,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_circuittermination_type_tamper_is_blocked(self):
        """Submitting a different termination type/object while the current one is hidden cannot drop or remap it."""
        self.add_permissions('circuits.view_circuit')
        provider = Provider.objects.create(name='Provider 1', slug='provider-1')
        circuit_type = CircuitType.objects.create(name='Type 1', slug='type-1')
        circuit = Circuit.objects.create(provider=provider, type=circuit_type, cid='Circuit 1')
        ...
        form = CircuitTerminationForm(
            ...
            data={
                ...
                'termination_content_type': providernetwork_ct.pk, 'termination_object_id': '',
            },
            instance=termination,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_circuitgroupassignment_type_tamper_is_blocked(self):
        """Submitting a different member type/object while the current member is hidden cannot drop or remap it."""
        self.add_permissions('circuits.view_circuitgroup')
        provider = Provider.objects.create(name='Provider 1', slug='provider-1')
        circuit_type = CircuitType.objects.create(name='Type 1', slug='type-1')
        circuit = Circuit.objects.create(provider=provider, type=circuit_type, cid='Circuit 1')
        ...
        form = CircuitGroupAssignmentForm(
            ...
            data={
                'group': group.pk,
                'member_content_type': virtualcircuit_ct.pk, 'member_object_id': '',
            },
            instance=assignment,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_vlangroup_scope_type_blank_tamper_is_blocked(self):
        """Blanking the scope subwidgets cannot drop a hidden scope."""
        site = Site.objects.create(name='Scope Site', slug='scope-site')
        group = VLANGroup.objects.create(name='Group 1', slug='group-1', scope=site)

        form = VLANGroupForm(
            data={
                'name': 'Group 1', 'slug': 'group-1', 'vid_ranges': '1-100',
                'scope_content_type': '', 'scope_object_id': '',
            },
            instance=group,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_circuitgroupassignment_member_type_blank_tamper_is_blocked(self):
        """Blanking the member subwidgets cannot drop a hidden member circuit."""
        self.add_permissions('circuits.view_circuitgroup')
        provider = Provider.objects.create(name='Provider 1', slug='provider-1')
        circuit_type = CircuitType.objects.create(name='Type 1', slug='type-1')
        circuit = Circuit.objects.create(provider=provider, type=circuit_type, cid='Circuit 1')
        ...
        form = CircuitGroupAssignmentForm(
            data={'group': group.pk, 'member_content_type': '', 'member_object_id': ''},
            instance=assignment,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_prefix_scope_blank_tamper_is_blocked(self):
        """PrefixForm inherits the scope selector from ScopedForm via the MRO; blanking scope cannot drop it."""
        site = Site.objects.create(name='Scope Site', slug='scope-site')
        prefix = Prefix.objects.create(prefix='10.0.0.0/24', scope=site)

        form = PrefixForm(
            data={'prefix': '10.0.0.0/24', 'status': 'active', 'scope_content_type': '', 'scope_object_id': ''},
            instance=prefix,
        )
        restrict_form_fields(form, self.user)
    ...
    def test_wirelesslan_scope_blank_tamper_is_blocked(self):
        """WirelessLANForm inherits the scope selector from ScopedForm; blanking scope cannot drop it."""
        site = Site.objects.create(name='Scope Site', slug='scope-site')
        wireless_lan = WirelessLAN.objects.create(ssid='WLAN 1', scope=site)

        form = WirelessLANForm(
            data={'ssid': 'WLAN 1', 'status': 'active', 'scope_content_type': '', 'scope_object_id': ''},
    ...
    def test_cluster_scope_blank_tamper_is_blocked(self):
        """ClusterForm inherits the scope selector from ScopedForm; blanking scope cannot drop it."""
        ...
        form = ClusterForm(
            data={
                'name': 'Cluster 1', 'type': cluster_type.pk, 'status': 'active',
                'scope_content_type': '', 'scope_object_id': '',
            },
            instance=cluster,
        )
